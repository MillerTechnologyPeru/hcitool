...
public enum Command {
    ...
    // Controls which LE events are generated by the HCI for the Host.
    case setEventMask(SetEventMaskCommand)
    
    // Reads the maximum size of the data portion of HCI LE ACL Data Packets sent from the Host to the Controller.
    case setReadBufferSize
}
...
public extension Command {
    ...
    public func execute <Controller: BluetoothHostControllerInterface> (controller: Controller) throws {
        ...
        case let .setEventMask(command): try command.execute(controller: controller)
        case .setReadBufferSize: try ReadBufferSizeCommand().execute(controller: controller)
        }
    }
}
...
public extension Command {
    ...
    public init(arguments: [String]) throws {
        ...
        case .setEventMask:
            let options = try SetEventMaskCommand.Option.parse(arguments: commandArguments)
            let commandValue = try SetEventMaskCommand(options: options)
            self = .setEventMask(commandValue)
        case .setReadBufferSize:
            self = .setReadBufferSize
        }
    }
}
